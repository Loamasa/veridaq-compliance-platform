...
const TranslationsView: React.FC<TranslationsViewProps> = ({ quickAction }) => {
  ...
  const getTranslationStatus = (postId: string, langCode: string) =>
    translations.find((t) => t.post_id === postId && t.language_code === langCode);
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto bg-gradient-to-br from-white via-neutral-50/30 to-white min-h-screen">
      <div className="flex items-center justify-between mb-12">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-neutral-900 via-primary-700 to-neutral-900 bg-clip-text text-transparent">
              ...
              <div className="flex items-start justify-between gap-6 mb-8">
                ...
                <div className="flex items-start gap-4">
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {isTokenWarning && (
                      <div className="flex items-center gap-1 text-xs text-warning-600 bg-warning-50 px-2 py-1 rounded-lg border warning-200">
                        <AlertTriangle className="w-3 h-3" /> Exceeds max output
                      </div>
                    )}
                  </div>
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className={`text-sm font-semibold truncate ${translation?.translation_status === 'completed' ? 'text-success-700' : translation?.translation_status === 'pending' || isTranslating ? 'text-warning-700' : translation?.translation_status === 'failed' ? 'text-error-700' : 'text-neutral-600'}`}>{lang.name}</div>
                          <div className={`text-xs flex items-center gap-1 ${translation?.translation_status === 'completed' ? 'text-success-600' : isTranslating || isRetrying ? 'text-warning-600' : translation?.translation_status === 'failed' ? 'text-error-600' : 'text-neutral-500'}`}>
                            <span>
                        ...
                        {translation?.translation_status === 'completed' ? (
                          <>
                            ...
                            <button
                              ...
                              disabled={isUpdating}
                              className={`p-2 transition-all duration-200 rounded-lg hover:scale-110 flex-shrink-0 ${translation.published === true ? 'text-warning-600 hover:text-warning-700 hover:bg-warning-200/50' : 'text-success-600 hover:text-success-700 hover:bg-success-200/50'}`}
